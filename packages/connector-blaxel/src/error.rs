...
#[derive(Debug, Error)]
pub enum BlaxelConnectorError {
    #[error("Blaxel connector configuration is invalid: {0}")]
    InvalidConfiguration(String),
    #[error(transparent)]
    Transport(#[from] BlaxelTransportError),
}

#[derive(Clone, Debug, Error)]
#[error("{message}")]
pub struct BlaxelTransportError {
    pub message: String,
    pub retryable: bool,
    pub disconnected: bool,
}

impl BlaxelTransportError {
...
pub(crate) fn transport_execution_error(source: BlaxelTransportError) -> ExecutionError {
    ExecutionError {
