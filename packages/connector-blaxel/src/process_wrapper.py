...
def main():
    ...
    # Persist the outer wrapper PID beside the journal so a new connector
    # instance can verify and reconnect to Blaxel's named process.
    ...
    try:
        ...
        write_ids = set()
    control_directory = os.path.join(directory, "control")
    ...
    while child.poll() is None or open_outputs:
        if deadline is not None and time.monotonic() >= deadline and child.poll() is None:
            ...
            cancelled = True
        for name in sorted(os.listdir(control_directory)):
            if not name.endswith(".json"):
                continue
            path = os.path.join(control_directory, name)
            try:
                with open(path, "r", encoding="utf-8") as handle:
                    apply_control(json.load(handle))
                os.unlink(path)
            except FileNotFoundError:
                pass
