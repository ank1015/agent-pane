...
use provider_openrouter::{OpenRouterConfig, OpenRouterProvider};
...
#[tokio::test]
async fn complete_performs_an_attributed_non_streaming_http_round_trip() {
    let native = json!({
        "id": "gen-http",
    ...
    let config = OpenRouterConfig::new("secret-key")
        .expect("valid config")
        .with_base_url(format!("{base_url}/api/v1"))
        .expect("valid test URL")
        .with_http_referer("https://agent-pane.example")
        .expect("valid referer")
        .with_app_title("Agent Pane")
        .expect("valid title")
        .with_router_metadata(true);
    let provider = OpenRouterProvider::new(config).expect("valid provider");
    ...
    assert!(lower_headers.contains("http-referer: https://agent-pane.example\r\n"));
    assert!(lower_headers.contains("x-openrouter-title: agent pane\r\n"));
    assert!(lower_headers.contains("x-openrouter-metadata: enabled\r\n"));
    assert_eq!(body["model"], GEMINI);
    assert_eq!(body["stream"], false);
    assert_eq!(message.native_message, native);
    assert_eq!(
        message.usage.expect("usage").cost.expect("cost").total,
        0.00001
    );
}

#[tokio::test]
async fn complete_normalizes_openrouter_http_errors() {
    ...
    assert_eq!(error.provider_code.as_deref(), Some("google-429"));
