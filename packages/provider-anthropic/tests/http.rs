use std::collections::BTreeMap;

use llm_contracts::{
    ContentPart, LlmRequest, LlmTransport, Message, MessageId, ModelId, ModelRef, ProviderId,
    TextContent, Timestamp, UserMessage,
};
use provider_anthropic::{AnthropicConfig, AnthropicProvider};
use serde_json::{Map, Value, json};
use tokio::{
    io::{AsyncReadExt, AsyncWriteExt},
    net::TcpListener,
    task::JoinHandle,
};

fn request(model_id: &str) -> LlmRequest {
    LlmRequest {
        model: ModelRef {
            provider: ProviderId::new("anthropic").expect("valid provider id"),
            id: ModelId::new(model_id).expect("valid model id"),
            name: None,
        },
        instructions: None,
        messages: vec![Message::User(UserMessage {
            id: MessageId::new("user-http").expect("valid id"),
            timestamp: Timestamp(0),
            content: vec![ContentPart::Text(TextContent {
                content: "Hello".into(),
                metadata: None,
            })],
        })],
        tools: Vec::new(),
        provider_options: Map::new(),
        metadata: BTreeMap::new(),
    }
}

#[tokio::test]
async fn complete_performs_a_non_streaming_http_round_trip() {
    let native = json!({
        "id": "msg_http",
        "type": "message",
        "role": "assistant",
        "model": "claude-sonnet-5",
        "content": [{ "type": "text", "text": "Hello" }],
        "stop_reason": "end_turn",
        "stop_sequence": null,
        "usage": { "input_tokens": 10, "output_tokens": 2 }
    });
    let (base_url, captured) = spawn_server("200 OK", &[], native.clone()).await;
    let config = AnthropicConfig::new("secret-key")
        .expect("valid config")
        .with_base_url(format!("{base_url}/v1"))
        .expect("valid test URL")
        .with_beta_header("test-beta-20260822")
        .expect("valid beta header");
    let provider = AnthropicProvider::new(config).expect("valid provider");

    let message = provider
        .complete(request("claude-sonnet-5"))
        .await
        .expect("successful completion");
    let raw_request = captured.await.expect("mock server completed");
    let lower_headers = raw_request.to_ascii_lowercase();
    let body: Value = serde_json::from_str(
        raw_request
            .split_once("\r\n\r\n")
            .expect("HTTP request contains a body")
            .1,
    )
    .expect("request body is JSON");

    assert!(raw_request.starts_with("POST /v1/messages HTTP/1.1\r\n"));
    assert!(lower_headers.contains("x-api-key: secret-key\r\n"));
    assert!(lower_headers.contains("anthropic-version: 2023-06-01\r\n"));
    assert!(lower_headers.contains("anthropic-beta: test-beta-20260822\r\n"));
    assert_eq!(body["stream"], false);
    assert_eq!(body["max_tokens"], 8_192);
    assert_eq!(message.native_message, native);
    assert!(message.usage.expect("usage").cost.is_some());
}

#[tokio::test]
async fn complete_supports_explicit_bearer_auth_without_an_api_key_header() {
    let native = json!({
        "id": "msg_bearer",
        "type": "message",
        "role": "assistant",
        "model": "claude-sonnet-5",
        "content": [{ "type": "text", "text": "Hello" }],
        "stop_reason": "end_turn",
        "usage": { "input_tokens": 1, "output_tokens": 1 }
    });
    let (base_url, captured) = spawn_server("200 OK", &[], native).await;
    let config = AnthropicConfig::from_bearer_token("aws-secret")
        .expect("valid config")
        .with_base_url(format!("{base_url}/v1"))
        .expect("valid test URL");
    let provider = AnthropicProvider::new(config).expect("valid provider");

    provider
        .complete(request("claude-sonnet-5"))
        .await
        .expect("successful completion");
    let raw_request = captured.await.expect("mock server completed");
    let lower_headers = raw_request.to_ascii_lowercase();

    assert!(lower_headers.contains("authorization: bearer aws-secret\r\n"));
    assert!(!lower_headers.contains("x-api-key:"));
}

#[tokio::test]
async fn complete_preserves_and_normalizes_anthropic_errors() {
    let native_error = json!({
        "type": "error",
        "error": { "type": "rate_limit_error", "message": "Slow down" },
        "request_id": "req_123"
    });
    let (base_url, captured) = spawn_server(
        "429 Too Many Requests",
        &[("retry-after", "1.5")],
        native_error.clone(),
    )
    .await;
    let config = AnthropicConfig::new("secret-key")
        .expect("valid config")
        .with_base_url(format!("{base_url}/v1"))
        .expect("valid test URL");
    let provider = AnthropicProvider::new(config).expect("valid provider");

    let error = provider
        .complete(request("claude-opus-5"))
        .await
        .expect_err("rate limit must fail");
    captured.await.expect("mock server completed");

    assert_eq!(error.message, "Slow down");
    assert_eq!(error.http_status, Some(429));
    assert_eq!(error.provider_type.as_deref(), Some("rate_limit_error"));
    assert_eq!(error.provider_code.as_deref(), Some("req_123"));
    assert_eq!(error.retry_after_ms, Some(1_500));
    assert_eq!(error.native_error.as_deref(), Some(&native_error));
    assert!(error.can_retry);
}

async fn spawn_server(
    status: &str,
    headers: &[(&str, &str)],
    body: Value,
) -> (String, JoinHandle<String>) {
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("bind mock server");
    let address = listener.local_addr().expect("mock server address");
    let status = status.to_owned();
    let headers = headers
        .iter()
        .map(|(name, value)| ((*name).to_owned(), (*value).to_owned()))
        .collect::<Vec<_>>();
    let body = serde_json::to_string(&body).expect("serializable mock body");
    let handle = tokio::spawn(async move {
        let (mut socket, _) = listener.accept().await.expect("accept HTTP request");
        let mut request = Vec::new();
        let mut buffer = [0_u8; 4_096];
        let mut expected_length = None;
        loop {
            let read = socket.read(&mut buffer).await.expect("read HTTP request");
            if read == 0 {
                break;
            }
            request.extend_from_slice(&buffer[..read]);
            if expected_length.is_none()
                && let Some(header_end) = find_bytes(&request, b"\r\n\r\n")
            {
                let header_text = String::from_utf8_lossy(&request[..header_end]);
                let content_length = header_text
                    .lines()
                    .find_map(|line| {
                        let (name, value) = line.split_once(':')?;
                        name.eq_ignore_ascii_case("content-length")
                            .then(|| value.trim().parse::<usize>().ok())
                            .flatten()
                    })
                    .unwrap_or(0);
                expected_length = Some(header_end + 4 + content_length);
            }
            if expected_length.is_some_and(|length| request.len() >= length) {
                break;
            }
        }

        let mut response = format!(
            "HTTP/1.1 {status}\r\ncontent-type: application/json\r\ncontent-length: {}\r\nconnection: close\r\n",
            body.len()
        );
        for (name, value) in headers {
            response.push_str(&format!("{name}: {value}\r\n"));
        }
        response.push_str("\r\n");
        response.push_str(&body);
        socket
            .write_all(response.as_bytes())
            .await
            .expect("write HTTP response");
        String::from_utf8(request).expect("HTTP request is UTF-8")
    });
    (format!("http://{address}"), handle)
}

fn find_bytes(haystack: &[u8], needle: &[u8]) -> Option<usize> {
    haystack
        .windows(needle.len())
        .position(|window| window == needle)
}
