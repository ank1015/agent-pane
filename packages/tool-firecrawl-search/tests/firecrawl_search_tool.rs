...
#[tokio::test]
#[ignore = "requires packages/tool-firecrawl-search/.env and consumes Firecrawl credits"]
async fn live_search_reaches_firecrawl() {
    let context = FirecrawlSearchToolContext::from_package_env()
        .expect("package-local Firecrawl credentials");
    let output = execute(
        FirecrawlSearchArguments {
