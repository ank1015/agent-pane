...
impl DeepSeekProvider {
    ...
    pub fn from_api_key(api_key: impl Into<String>) -> Result<Self, LlmError> {
        Self::new(DeepSeekConfig::new(api_key)?)
    }
    ...
    pub fn with_client(config: DeepSeekConfig, client: reqwest::Client) -> Self {
        Self {
            client,
            config,
            provider_id: ProviderId::new(DEEPSEEK_PROVIDER)
                .expect("the built-in DeepSeek provider id is valid"),
        }
    }
...
#[async_trait]
impl LlmTransport for DeepSeekProvider {
    async fn complete(&self, request: LlmRequest) -> Result<AssistantMessage, LlmError> {
        ...
        let native = serde_json::from_str(&response_body).map_err(|error| LlmError {
            message: format!("DeepSeek returned invalid JSON: {error}"),
            provider_code: None,
...
impl LlmProviderAdapter for DeepSeekProvider {
...
fn bearer_header(api_key: &str) -> Result<HeaderValue, LlmError> {
    let mut value = HeaderValue::from_str(&format!("Bearer {api_key}"))
        .map_err(|_| invalid_config("DeepSeek API key contains invalid header characters."))?;
