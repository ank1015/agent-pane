...
#[derive(Clone)]
pub(crate) struct E2bProcessRuntime {
...
impl E2bProcessRuntime {
    ...
    async fn hydrate(&self, execution_id: &ExecutionId) -> ExecutionResult<Arc<ProcessEntry>> {
        ...
        let expected_tag = format!("{PROCESS_TAG_PREFIX}{execution_id}");
        let pid = processes
            .iter()
            .find(|process| process.tag.as_deref() == Some(&expected_tag))
    ...
    async fn send_control(&self, entry: &ProcessEntry, value: Value) -> ExecutionResult<()> {
        ...
        self.inner
            .transport
            .send_input(entry.pid, false, &bytes)
            .await
            .map_err(transport_execution_error)
    }
}
...
#[async_trait]
impl ProcessRuntime for E2bProcessRuntime {
    async fn start(
        &self,
        ...
        if request.policy.network == NetworkMode::Denied {
            return Err(execution_error(
                ...
                "E2B cannot securely deny network access for only one process; configure the sandbox network instead",
            ));
        }
        if request.policy.profile.is_some() {
            return Err(execution_error(
                ExecutionErrorCode::Unsupported,
                "E2B process policy profiles are unsupported",
            ));
        }
            ...
            match remote.next().await {
                ...
                None => {
                    return Err(execution_error(
                        ...
                        "E2B process stream ended before start",
                    ));
                }
            }
        };
