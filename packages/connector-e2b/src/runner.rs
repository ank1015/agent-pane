        ...
        Response: DeserializeOwned,
    {
        ...
        if argument.len() > MAX_INLINE_ARGUMENT_BYTES {
            return Err(execution_error(
                ...
                "inline E2B operation exceeds the safe process argument limit; use artifact-backed content",
            ));
        }
        let mut stream = self
            .transport
        ...
        if exit_code != Some(0) {
            return Err(execution_error(
                ...
                format!(
                    "inline E2B Python operation exited with {:?}: {}",
                    exit_code,
        ...
        let envelope: RunnerResponse = serde_json::from_slice(&stdout).map_err(|source| {
            execution_error(
                ExecutionErrorCode::Internal,
                format!(
                    "invalid inline E2B response: {source}; stderr={}",
                    String::from_utf8_lossy(&stderr)
                ),
            )
        })?;
        if envelope.ok {
            serde_json::from_value(envelope.value).map_err(|source| {
                execution_error(
                    ExecutionErrorCode::Internal,
                    format!("inline E2B result does not match its contract: {source}"),
                )
            })
        } else {
            Err(envelope.error.unwrap_or_else(|| {
                execution_error(
                    ExecutionErrorCode::Internal,
                    "failed inline E2B response omitted its error",
                )
            }))
        }
    }
