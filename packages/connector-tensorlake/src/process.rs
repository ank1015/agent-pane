...
#[derive(Clone)]
pub(crate) struct TensorlakeProcessRuntime {
            ...
            .filter(|pid| {
                processes.iter().any(|process| {
                    process.pid == *pid
                        ...
                        && process.arguments.iter().any(|arg| arg == PROCESS_WRAPPER)
                })
            })
            .unwrap_or(0);
...
#[async_trait]
impl ProcessRuntime for TensorlakeProcessRuntime {
    async fn start(
        &self,
        ...
        if request.policy.network == NetworkMode::Denied {
            return Err(execution_error(
                ...
                "Tensorlake cannot securely deny network access for only one process; configure the sandbox network instead",
            ));
        }
        if request.policy.profile.is_some() {
            return Err(execution_error(
                ExecutionErrorCode::Unsupported,
                "Tensorlake process policy profiles are unsupported",
            ));
        }
            ...
            match remote.next().await {
                ...
                None => {
                    return Err(execution_error(
                        ...
                        "Tensorlake process stream ended before start",
                    ));
                }
            }
        };
