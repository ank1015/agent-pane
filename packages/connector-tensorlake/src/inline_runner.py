...
class Runtime:
    ...
    def resolve(self, spec, write=False, follow=True):
        ...
        if kind == "workspace":
            ...
            if uri.scheme != "file" or uri.netloc not in ("", "localhost"):
                raise RunnerError("unsupported", "Tensorlake supports only file:// native paths")
    ...
    def preview_plan(self, plan):
        if plan.get("post_actions", {}).get("format", "none") != "none" or plan.get("post_actions", {}).get("diagnostics", "none") != "none":
            raise RunnerError("unsupported", "Tensorlake inline mutations do not yet provide formatter or diagnostics post-actions")
    ...
    def op_process_replay(self):
        ...
        runner_pid = None
        try:
            with open(os.path.join(directory, "runner.json"), "r", encoding="utf-8") as handle:
                runner_pid = json.load(handle).get("pid")
        except FileNotFoundError:
            pass
        return {"events": events, "status": status, "runner_pid": runner_pid}
