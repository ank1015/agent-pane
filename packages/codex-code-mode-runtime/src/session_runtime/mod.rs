...
use crate::CodeModeStateStore;
use crate::InMemoryCodeModeStateStore;
...
impl<D: SessionRuntimeDelegate> SessionRuntime<D> {
    pub(crate) fn new(delegate: Arc<D>) -> Self {
        Self::new_with_state_store_and_task_failure_handler(
            delegate,
    ...
    pub(crate) fn new_with_state_store_and_task_failure_handler(
        delegate: Arc<D>,
        state_store: Arc<dyn CodeModeStateStore>,
        task_failure_handler: Option<TaskFailureHandler>,
    ) -> Self {
    ...
    pub(crate) async fn execute(
        ...
        let cell_id = self.allocate_cell_id().await?;
    ...
    async fn allocate_cell_id(&self) -> Result<CellId, Error> {
        self.inner
            .state_store
            .allocate_cell_id()
            .await
            .map(|cell_id| CellId::new(cell_id.as_str()))
...
impl<D: SessionRuntimeDelegate> CellHost for RuntimeCellHost<D> {
    ...
    async fn commit_completion(
        ...
        if cell_state.cancellation_token().is_cancelled() {
            return CompletionCommit::Rejected(event);
        }
        let event = match self
            .inner
            .state_store
            .commit_values(stored_value_writes)
            .await
        {
            Ok(()) => event,
            Err(error) => completion_persistence_error(event, error),
        };
...
fn completion_persistence_error(event: CellEvent, error: String) -> CellEvent {
    match event {
        CellEvent::Completed { content_items, .. } => CellEvent::Completed {
            content_items,
            error_text: Some(format!("failed to persist code mode state: {error}")),
        },
        other => other,
    }
}
