...
async fn migration_builds_the_schema_and_enforces_aggregate_boundaries() {
    ...
    let harness_supported_providers_exists: bool = sqlx::query_scalar(
        "select exists(
             select 1 from information_schema.columns
             where table_schema = 'public' and table_name = 'harnesses'
               and column_name = 'supported_providers' and data_type = 'jsonb'
         )",
    )
    .fetch_one(&pool)
    .await
    .expect("harnesses supported_providers inspection");
    assert!(harness_supported_providers_exists);
    let harness_supported_reasoning_levels_exists: bool = sqlx::query_scalar(
        "select exists(
             select 1 from information_schema.columns
             where table_schema = 'public' and table_name = 'harnesses'
               and column_name = 'supported_reasoning_levels' and data_type = 'jsonb'
         )",
    )
    .fetch_one(&pool)
    .await
    .expect("harnesses supported_reasoning_levels inspection");
    assert!(harness_supported_reasoning_levels_exists);
