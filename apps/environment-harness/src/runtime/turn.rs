...
use crate::{
    ...
    harness::{
        compact::{
            ...
            form_compaction_request, select_first_kept_message_id,
        },
        context_formation::{ContextFormationError, form_context_messages},
        model_resolver::{RequestKind, get_model_config},
        system_prompt::generate_system_prompt,
        tools::{ToolExecutionContext, default_tool_definitions, execute_tool_call},
    },
};

use super::{
    config::EnvironmentHarnessConfig,
    error::{EnvironmentRuntimeBuildError, EnvironmentRuntimeError, TurnError},
...
impl EnvironmentRuntime {
    pub fn from_config(config: &HarnessConfig) -> Result<Self, EnvironmentRuntimeBuildError> {
        Ok(Self::new(
    ...
    pub async fn execute(&self, run: &ActiveTurn) -> Result<TurnOutcome, EnvironmentRuntimeError> {
    ...
    async fn execute_turn(&self, run: &ActiveTurn) -> Result<TurnDecision, TurnError> {
        ensure_active(run)?;
        let config = EnvironmentHarnessConfig::from_resolved(&run.request().resolved_config)?;
        let messages = fetch_messages(run).await?;
        let turn = run.turn_number();
        match plan_turn(&messages, run.run_id(), turn)? {
            ResumePlan::CallModel => self.call_model(run, &config, messages).await,
            ...
            ResumePlan::ExecuteTools {
                ...
                self.run_tools(run, &assistant, &missing_tool_calls, config.project_id)
                    .await?;
        ...
        loop {
            ...
            .await
            {
                Ok(assistant) => {
                    ...
                    self.run_tools(run, &assistant, &tool_calls, config.project_id)
                        .await?;
    ...
    async fn compact(
        ...
        let session_message_id = Uuid::now_v7();
        let message = Message::Custom(create_environment_compaction_message(
            MessageId::new(format!("environment-compaction-{session_message_id}"))
    ...
    async fn run_tools(
        ...
        let results = if assistant.stop_reason == StopReason::Length {
            ...
            for tool_call in tool_calls {
                results.push(execute_tool_call(tool_call, &context).await?);
            }
            results
        };
    ...
    fn handle_error(
        ...
        run: &ActiveTurn,
        error: TurnError,
    ) -> Result<TurnOutcome, EnvironmentRuntimeError> {
        if run.operation().is_cancelled() {
        ...
        if error.agent_retryable() {
            ...
            return Err(EnvironmentRuntimeError::Agent(source));
        }
