...
pub const ENVIRONMENT_COMPACTION_MESSAGE_TAG: &str = "environment.compaction";
...
pub struct EnvironmentCompactionMessageContent {
    pub summary: String,
...
pub fn create_environment_compaction_message(
    id: MessageId,
    timestamp: Timestamp,
    compaction: EnvironmentCompactionMessageContent,
    ...
    let serde_json::Value::Object(content) = content else {
        unreachable!("EnvironmentCompactionMessageContent serializes as an object")
    };

    Ok(CustomMessage {
        id,
        content,
        tag: Some(ENVIRONMENT_COMPACTION_MESSAGE_TAG.to_owned()),
        timestamp,
    })
}
...
pub fn form_compaction_request(
    ...
    Ok(LlmRequest {
        ...
        messages: vec![Message::User(UserMessage {
            id: MessageId::new("environment-compaction-request")
                .expect("static message id is valid"),
            timestamp,
...
fn visible_message_indices(messages: &[Message]) -> Vec<usize> {
    let Some((compaction_index, compaction)) = latest_compaction(messages) else {
        return messages
            .iter()
            .enumerate()
            .filter_map(|(index, message)| (!is_environment_compaction(message)).then_some(index))
            .collect();
    };
    ...
    (first_kept..messages.len())
        .filter(|index| *index != compaction_index && !is_environment_compaction(&messages[*index]))
        .collect()
}

fn is_valid_cut_point(message: &Message) -> bool {
    !matches!(message, Message::ToolResult(_)) && !is_environment_compaction(message)
}

fn is_environment_compaction(message: &Message) -> bool {
    matches!(
        message,
        Message::Custom(message) if message.tag.as_deref() == Some(ENVIRONMENT_COMPACTION_MESSAGE_TAG)
    )
}

pub(crate) fn latest_compaction(
    messages: &[Message],
) -> Option<(usize, EnvironmentCompactionMessageContent)> {
    messages
        .iter()
        .enumerate()
        .rev()
...
mod tests {
    ...
    use super::{
        COMPACTION_MAX_OUTPUT_TOKENS, COMPACTION_SYSTEM_PROMPT, ENVIRONMENT_COMPACTION_MESSAGE_TAG,
        EnvironmentCompactionMessageContent, create_environment_compaction_message,
        form_compaction_request, generate_compaction_system_prompt, serialize_message_history,
    };

    #[test]
    fn returns_environment_compaction_system_prompt() {
        assert_eq!(
    ...
    fn serializes_compaction_message_content() {
        ...
        let message = create_environment_compaction_message(
        ...
        assert_eq!(ENVIRONMENT_COMPACTION_MESSAGE_TAG, "environment.compaction");
        assert_eq!(
            serde_json::to_value(Message::Custom(message)).expect("compaction message serializes"),
            json!({
                "role": "custom",
                "id": "compaction-1",
                "tag": "environment.compaction",
                "timestamp": 1_000,
                "content": {
...
[User]: Work after the current compaction."
    ...
    fn forms_a_complete_uncached_compaction_request() {
        ...
        assert_eq!(user_message.id.as_str(), "environment-compaction-request");
