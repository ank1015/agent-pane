...
impl HarnessConfig {
        ...
        F: FnMut(&str) -> Option<String>,
    {
        let instance_id = validated_id(
            "ENVIRONMENT_HARNESS_INSTANCE_ID",
            lookup("ENVIRONMENT_HARNESS_INSTANCE_ID")
                .unwrap_or_else(|| format!("environment-harness-{}", Uuid::now_v7())),
        )?;
        let max_concurrent_turns = positive_usize(
            "ENVIRONMENT_HARNESS_MAX_CONCURRENT_TURNS",
            lookup("ENVIRONMENT_HARNESS_MAX_CONCURRENT_TURNS"),
            20,
        )?;
        let agent_url = http_url(
            "ENVIRONMENT_HARNESS_AGENT_URL",
            lookup("ENVIRONMENT_HARNESS_AGENT_URL")
                .as_deref()
                .unwrap_or(DEFAULT_AGENT_URL),
        )?;
        let agent_timeout = seconds(
            "ENVIRONMENT_HARNESS_AGENT_TIMEOUT_SECONDS",
            lookup("ENVIRONMENT_HARNESS_AGENT_TIMEOUT_SECONDS"),
            30,
        )?;
        let harness_registration = optional(
            "ENVIRONMENT_HARNESS_AGENT_CONTROL_TOKEN",
            lookup("ENVIRONMENT_HARNESS_AGENT_CONTROL_TOKEN"),
        )?
        ...
        let agent = AgentServiceConfig {
            ...
            harness_token: required(
                "ENVIRONMENT_HARNESS_AGENT_TOKEN",
                lookup("ENVIRONMENT_HARNESS_AGENT_TOKEN"),
            )?,
            request_timeout: agent_timeout,
        };
        let llm_gateway = LlmGatewayServiceConfig {
            base_url: http_url(
                "ENVIRONMENT_HARNESS_LLM_GATEWAY_URL",
                lookup("ENVIRONMENT_HARNESS_LLM_GATEWAY_URL")
                    .as_deref()
                    .unwrap_or(DEFAULT_LLM_GATEWAY_URL),
            )?,
            request_timeout: seconds(
                "ENVIRONMENT_HARNESS_LLM_TIMEOUT_SECONDS",
                lookup("ENVIRONMENT_HARNESS_LLM_TIMEOUT_SECONDS"),
                36 * 60,
            )?,
        };
        let mut execution_gateway = ExecutionGatewayConfig::new(
            http_url(
                "ENVIRONMENT_HARNESS_EXECUTION_GATEWAY_URL",
                lookup("ENVIRONMENT_HARNESS_EXECUTION_GATEWAY_URL")
                    .as_deref()
                    .unwrap_or(DEFAULT_EXECUTION_GATEWAY_URL),
            )?,
            required(
                "ENVIRONMENT_HARNESS_EXECUTION_GATEWAY_TOKEN",
                lookup("ENVIRONMENT_HARNESS_EXECUTION_GATEWAY_TOKEN"),
            )?,
        );
        execution_gateway.request_timeout = seconds(
            "ENVIRONMENT_HARNESS_EXECUTION_TIMEOUT_SECONDS",
            lookup("ENVIRONMENT_HARNESS_EXECUTION_TIMEOUT_SECONDS"),
            30,
        )?;
        ...
        Ok(Self {
            ...
            broker: BrokerConfig {
                url: lookup("ENVIRONMENT_HARNESS_NATS_URL")
                    .unwrap_or_else(|| "nats://127.0.0.1:4222".to_owned()),
                command_result_timeout: seconds(
                    "ENVIRONMENT_HARNESS_COMMAND_RESULT_TIMEOUT_SECONDS",
                    lookup("ENVIRONMENT_HARNESS_COMMAND_RESULT_TIMEOUT_SECONDS"),
                    30,
                )?,
                command_max_retries: positive_u32(
                    "ENVIRONMENT_HARNESS_COMMAND_MAX_RETRIES",
                    lookup("ENVIRONMENT_HARNESS_COMMAND_MAX_RETRIES"),
                    3,
                )?,
                command_retry_base: millis(
                    "ENVIRONMENT_HARNESS_COMMAND_RETRY_BASE_MILLISECONDS",
                    lookup("ENVIRONMENT_HARNESS_COMMAND_RETRY_BASE_MILLISECONDS"),
                    250,
                )?,
                command_retry_max: seconds(
                    "ENVIRONMENT_HARNESS_COMMAND_RETRY_MAX_SECONDS",
                    lookup("ENVIRONMENT_HARNESS_COMMAND_RETRY_MAX_SECONDS"),
                    10,
                )?,
                delivery_retry_delay: seconds(
                    "ENVIRONMENT_HARNESS_DELIVERY_RETRY_SECONDS",
                    lookup("ENVIRONMENT_HARNESS_DELIVERY_RETRY_SECONDS"),
                    5,
                )?,
                progress_interval: seconds(
                    "ENVIRONMENT_HARNESS_PROGRESS_INTERVAL_SECONDS",
                    lookup("ENVIRONMENT_HARNESS_PROGRESS_INTERVAL_SECONDS"),
                    30,
                )?,
            },
            agent,
...
mod tests {
    ...
    fn loads_server_defaults() {
        let config = load(&[
            ("ENVIRONMENT_HARNESS_AGENT_TOKEN", "agent-secret"),
            (
                "ENVIRONMENT_HARNESS_EXECUTION_GATEWAY_TOKEN",
                "execution-secret",
            ),
        ]);
        assert!(config.instance_id.starts_with("environment-harness-"));
    ...
    fn loads_broker_and_concurrency_overrides() {
        let config = load(&[
            ("ENVIRONMENT_HARNESS_INSTANCE_ID", "environment-a"),
            ("ENVIRONMENT_HARNESS_MAX_CONCURRENT_TURNS", "40"),
            ("ENVIRONMENT_HARNESS_NATS_URL", "nats://broker:4222"),
            ("ENVIRONMENT_HARNESS_AGENT_TOKEN", "agent-secret"),
            ("ENVIRONMENT_HARNESS_AGENT_CONTROL_TOKEN", "control-secret"),
            (
                "ENVIRONMENT_HARNESS_EXECUTION_GATEWAY_TOKEN",
                "execution-secret",
            ),
        ]);
        assert_eq!(config.instance_id, "environment-a");
