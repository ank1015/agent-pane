...
use environment_harness::{
    clients::{
        AgentClient, ENVIRONMENT_HARNESS_REVISION_ID, ExecutionClient, LlmGatewayClient,
        LlmGatewayClientError, ensure_environment_harness,
    },
...
#[tokio::test]
async fn registers_and_activates_the_packaged_environment_harness() {
    let calls = Arc::new(Mutex::new(Vec::new()));
    let app = Router::new()
    ...
    ensure_environment_harness(config)
        .await
    ...
    assert_eq!(calls[0].body["harness_id"], "environment");
    assert_eq!(
    ...
    assert!(calls[2].body["default_config"].get("is_replaced").is_none());
    assert!(
        calls[2].body["config_schema"]["properties"]
            .get("external_prompt")
            .is_none()
    );
    assert!(
    ...
    assert_eq!(
        calls[2].body["config_schema"]["properties"]["project_id"]["type"],
        "string"
    );
    assert_eq!(
