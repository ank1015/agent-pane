...
#[tokio::test]
#[ignore = "requires PI_HARNESS_TEST_NATS_URL pointing to an empty JetStream server"]
async fn consumes_a_turn_and_completes_the_command_round_trip_over_jetstream() {
    let nats_url = std::env::var("PI_HARNESS_TEST_NATS_URL").expect("test NATS URL");
    ...
    let HarnessOperation::Complete { final_message_id } = command.operation else {
        panic!("Pi should complete this turn");
    };
    assert_eq!(
        fixture
            .appended_message_id
...
fn config(nats_url: String, agent_url: Url, llm_url: Url) -> HarnessConfig {
    HarnessConfig {
        instance_id: "pi-broker-test".to_owned(),
