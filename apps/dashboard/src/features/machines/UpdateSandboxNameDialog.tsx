...
import {
  type SandboxMachine,
  useUpdateSandboxMachineName,
} from './machine-queries'

type UpdateSandboxNameDialogProps = {
  sandbox: SandboxMachine
  onClose: () => void
}

export function UpdateSandboxNameDialog({
  sandbox,
  onClose,
}: UpdateSandboxNameDialogProps) {
  const [name, setName] = useState(() => sandbox.name)
  const updateName = useUpdateSandboxMachineName()
  ...
  const confirmUpdate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateName.mutateAsync({ sandbox, name: normalizedName })
      updateName.reset()
      onClose()
    } catch {
    ...
    >
      <form id={formId} onSubmit={(event) => void confirmUpdate(event)}>
        <div className="update-provider-name-field">
          <label htmlFor={inputId}>Sandbox name</label>
          <input
            ref={inputRef}
            id={inputId}
            type="text"
