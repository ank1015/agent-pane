...
import {
  ...
  useUpdateSandboxEnvironmentTemplateName,
} from './machine-queries'

type UpdateEnvironmentTemplateNameDialogProps = {
  template: SandboxEnvironmentTemplate
  onClose: () => void
}

export function UpdateEnvironmentTemplateNameDialog({
  template,
  onClose,
}: UpdateEnvironmentTemplateNameDialogProps) {
  const [name, setName] = useState(() => template.name)
  const updateName = useUpdateSandboxEnvironmentTemplateName()
  ...
  const confirmUpdate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateName.mutateAsync({ template, name: normalizedName })
      updateName.reset()
      onClose()
    } catch {
