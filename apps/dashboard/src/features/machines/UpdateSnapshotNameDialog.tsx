...
import {
  ...
  useUpdateSnapshotName,
} from './machine-queries'

type UpdateSnapshotNameDialogProps = {
  snapshot: SandboxSnapshot
  onClose: () => void
}

export function UpdateSnapshotNameDialog({
  snapshot,
  onClose,
}: UpdateSnapshotNameDialogProps) {
  const [name, setName] = useState(() => snapshot.name)
  const updateName = useUpdateSnapshotName()
  ...
  const confirmUpdate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateName.mutateAsync({ snapshot, name: normalizedName })
      updateName.reset()
      onClose()
    } catch {
    ...
    >
      <form id={formId} onSubmit={(event) => void confirmUpdate(event)}>
        <div className="update-provider-name-field">
          <label htmlFor={inputId}>Snapshot name</label>
          <input
            ref={inputRef}
            id={inputId}
            type="text"
