...
const ConversationMessage = memo(function ConversationMessage({
  ...
  if (body.role === 'user') {
    return (
      <UserConversationMessage body={body} createdAt={message.created_at} />
    )
  }
...
const RunErrorMessage = memo(function RunErrorMessage({
  ...
  return (
    <div className="project-session-run-error" role="alert">
      ...
      <span>
        <strong>Couldn’t complete the environment run</strong>
        <span>{runFailureMessage(run.failure)}</span>
      </span>
    </div>
  )
})
...
function runDuration(run: ProjectRunSummary, currentTime?: number) {
  const activatedAt = Date.parse(run.activated_at)
