...
impl Vault {
    ...
    pub fn encrypt(
        ...
        let aad = associated_data(account_id, secret_id, provider);
...
fn associated_data(account_id: Uuid, secret_id: Uuid, provider: &str) -> String {
    format!("llm-gateway:v1:{account_id}:{secret_id}:{provider}")
}
...
pub enum VaultError {
    #[error("GATEWAY_MASTER_KEY must be valid base64")]
    InvalidMasterKeyEncoding,
    #[error("GATEWAY_MASTER_KEY must decode to exactly 32 bytes")]
...
mod tests {
    ...
    #[test]
    fn round_trip_is_bound_to_account_secret_and_provider() {
        let vault = Vault::from_base64(&STANDARD.encode([7_u8; 32])).unwrap();
        let account_id = Uuid::now_v7();
        let secret_id = Uuid::now_v7();
        let encrypted = vault
            .encrypt(account_id, secret_id, "openai", b"secret")
            .unwrap();
