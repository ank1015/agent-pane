...
use crate::{
    BasicCcToolsHarness, Config, prompt,
    state::{Phase, State},
...
pub(crate) async fn run(harness: BasicCcToolsHarness, execution: Execution) -> Result<()> {
...
impl Driver {
            ...
            match self.state.phase.clone() {
                ...
                Phase::Model {
                    ...
                    let result = if let Some(job) = job {
                        ...
                        let key =
                            IdempotencyKey::new(format!("basic-cc:{}:{operation}", self.run_id))
    ...
    async fn tool_result(
        ...
        if let Some(path) = output.consume {
            self.state.observations.retain(|old| old.path != path);
        }
        let result = tool_message(call, output.text, output.error);
        self.state.phase = Phase::Tools {
            assistant,
            index: index + 1,
            plan: None,
    ...
    async fn abort(&mut self) -> Result<()> {
        match self.state.phase.clone() {
            ...
            Phase::Tools {
                ...
                if let Plan::Bash { prepared, running } = *plan {
                    // Do not start a command just to stop it. Known IDs/generation
                    // allow reconciliation even if its start response was lost.
                    if self.host.is_none() {
                        self.host = Some(
                            self.harness
                                .execution
                                ...
                                .await
                                .map_err(|_| {
                                    Error::Invalid("cannot reach execution host for abort")
                                })?,
                        );
                    }
                    ...
                    if let Err(error) = result {
                        if !matches!(
                            ...
                            return Err(Error::Invalid("remote process abort unconfirmed"));
                        }
                    }
                }
            }
