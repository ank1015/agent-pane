...
use crate::{
    BasicCodexToolsHarness, Config, prompt,
...
pub(crate) async fn run(harness: BasicCodexToolsHarness, execution: Execution) -> Result<()> {
    ...
    let state = if let Some(checkpoint) = &context.checkpoint {
        ...
        let tools = Tools {
            ...
            cwd: cwd.clone(),
            policy: &policy,
            delivery: tool_view_image::ImageDelivery::Hosted(harness.publisher.clone()),
        };
...
struct Driver {
    harness: BasicCodexToolsHarness,
    client: RunClient,
...
impl Driver {
            ...
            match self.state.phase.clone() {
                ...
                Phase::Model {
                    ...
                    let result = if let Some(job) = job {
                        ...
                        let key =
                            IdempotencyKey::new(format!("basic-codex:{}:{operation}", self.run_id))
    ...
    async fn tool(&mut self, assistant: Uuid, index: usize, plan: Option<Box<Plan>>) -> Result<()> {
        ...
        let tools = Tools {
            ...
            delivery: tool_view_image::ImageDelivery::Hosted(self.harness.publisher.clone()),
