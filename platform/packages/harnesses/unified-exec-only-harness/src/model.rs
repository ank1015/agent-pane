        ...
        provider_fireworks::FIREWORKS_MODELS
            ...
            .filter(|model| {
                ["low", "medium", "high", "xhigh", "max"]
                    .iter()
                    .all(|level| provider_fireworks::reasoning_effort(model.id, level).is_some())
            })
...
pub fn provider_options(
    ...
    if model.provider.as_str() == "fireworks" {
        let model =
            provider_fireworks::find_model(model.id.as_str()).ok_or("Unknown Fireworks model")?;
        ...
        let effort = provider_fireworks::reasoning_effort(model.id, level)
            .ok_or("Missing reasoning mapping for Fireworks model")?;
        return Ok(json!({"reasoning_effort":effort,"prompt_cache_key":session.to_string(),"max_tokens":model.max_tokens}).as_object().unwrap().clone());
    }
    ...
    if !exists || !MODELS.contains(&model.id.as_str()) {
        return Err("Unsupported model or missing shell tool capability policy".into());
    }
    Ok(json!({
        "store": false,
