...
impl std::fmt::Debug for FirecrawlScrapeToolContext {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FirecrawlScrapeToolContext")
...
impl FirecrawlScrapeToolContext {
    ...
    pub fn from_env() -> Result<Self, FirecrawlScrapeToolError> {
        let key = env::var(API_KEY_ENV)
            .map_err(|_| FirecrawlScrapeToolError::configuration("FIRECRAWL_API_KEY is missing"))?;
        Self::new(key)
    }
    ...
    pub fn with_client(
            ...
            || endpoint.fragment().is_some()
        {
            return Err(FirecrawlScrapeToolError::configuration(
                "scrape endpoint must use HTTPS (or loopback HTTP) without credentials, query, or fragment",
            ));
        }
        Ok(Self {
            http,
...
impl FirecrawlScrapeToolError {
    ...
    fn request(error: reqwest::Error) -> Self {
        ...
        Self::new(
            name,
            format!("Webpage scrape request failed: {}", error.without_url()),
        )
    }
