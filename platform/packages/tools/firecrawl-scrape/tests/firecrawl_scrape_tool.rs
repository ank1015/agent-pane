...
async fn live_scrape_reaches_firecrawl() {
    let context = FirecrawlScrapeToolContext::from_env().expect("process credentials");
    let output = execute(
        FirecrawlScrapeArguments {
            url: "https://example.com/".to_owned(),
        },
...
fn credentials_are_redacted_and_endpoint_configuration_is_validated() {
    let context = FirecrawlScrapeToolContext::new("private-secret-marker").unwrap();
    assert!(!format!("{context:?}").contains("private-secret-marker"));
    for key in ["", " bad", "bad\nkey"] {
        assert!(FirecrawlScrapeToolContext::new(key).is_err());
    }
    for endpoint in [
        "http://example.com/v2/scrape",
        "https://user:pass@example.com/v2/scrape",
        "https://example.com/v2/scrape?key=secret",
        "file:///tmp/test",
    ] {
        assert!(FirecrawlScrapeToolContext::with_client("key", endpoint, Client::new()).is_err());
    }
}

#[tokio::test]
async fn oversized_model_text_and_metadata_are_bounded() {
    let (endpoint, captured) = spawn_server("200 OK", json!({"success":true,"data":{"markdown":"Useful page", "metadata":{"title":"é".repeat(90_000)}}})).await;
    let ctx =
        FirecrawlScrapeToolContext::with_client("secret-key", endpoint, Client::new()).unwrap();
    let output = execute(
        FirecrawlScrapeArguments {
    ...
    assert!(text(&output.content).len() <= tool_firecrawl_scrape::MAX_OUTPUT_TEXT_BYTES);
    ...
    assert!(
        serde_json::to_vec(&details).unwrap().len()
            <= tool_firecrawl_scrape::MAX_OUTPUT_DETAILS_BYTES
    );
    assert_eq!(details["truncated"], true);
}
...
async fn chunked_responses_are_limited_without_content_length() {
    ...
    let endpoint = format!("http://{}/v2/scrape", listener.local_addr().unwrap());
