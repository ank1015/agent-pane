...
export function RenameDeleteMenu({ name, onAction, triggerClassName = 'provider-actions-trigger' }: { name: string; onAction: (action: 'rename' | 'delete', trigger: HTMLButtonElement) => void; triggerClassName?: string }) {
  ...
  return <>
    <button ref={trigger} type="button" className={`cursor-button cursor-button--ghost cursor-icon-button ${triggerClassName}`}
    ...
    {open ? createPortal(<div ref={menu} id={menuId} role="menu" aria-label={`Actions for ${name}`} className="provider-actions-menu" style={position}
      onKeyDown={(event) => {
