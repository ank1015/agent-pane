...
export const ProjectRunDetailsDrawer = memo(function ProjectRunDetailsDrawer({
  ...
  const isActive = ['ready', 'running', 'waiting'].includes(run.status)
  const messageItems = useMemo(() => messages.filter(message => message.run_id === run.id), [messages, run.id])
  const transcriptItems = useMemo(() => buildRunTranscript(messageItems, run.final_message_id), [messageItems, run.final_message_id])
  useEffect(() => {
    if (!isClosing) return
    const timer = window.setTimeout(onClosed, 250)
  ...
  return messages.flatMap((message) => {
    ...
    return body.content.flatMap((part, index) =>
      transcriptAssistantPart(
        ...
        message,
        index,
        message.message_id === finalMessageId,
        toolResults,
      ),
    )
  })
...
function transcriptAssistantPart(
  ...
  const id = `${message.message_id}:${part.type}:${index}`
