...
/// Non-streaming Fireworks Chat Completions client.
#[derive(Clone, Debug)]
...
impl FireworksProvider {
    ...
    pub fn new(config: FireworksConfig) -> Result<Self, LlmError> {
    ...
    pub fn from_api_key(api_key: impl Into<String>) -> Result<Self, LlmError> {
        Self::new(FireworksConfig::new(api_key)?)
    }

    /// Creates a provider with an application-owned HTTP client.
    #[must_use]
    pub fn with_client(config: FireworksConfig, client: reqwest::Client) -> Self {
        Self {
            client,
            config,
            provider_id: ProviderId::new(FIREWORKS_PROVIDER)
                .expect("the built-in Fireworks provider id is valid"),
        }
    }
...
#[async_trait]
impl LlmTransport for FireworksProvider {
    async fn complete(&self, request: LlmRequest) -> Result<AssistantMessage, LlmError> {
        ...
        let native =
            serde_json::from_str(&response_body).map_err(|error| llm_contracts::LlmError {
                message: format!("Fireworks returned invalid JSON: {error}"),
                provider_code: None,
...
impl LlmProviderAdapter for FireworksProvider {
...
fn bearer_header(api_key: &str) -> Result<HeaderValue, LlmError> {
    let mut value = HeaderValue::from_str(&format!("Bearer {api_key}"))
        .map_err(|_| invalid_config("Fireworks API key contains invalid header characters."))?;
...
async fn read_response_body(response: &mut reqwest::Response) -> Result<String, LlmError> {
    ...
    String::from_utf8(body).map_err(|_| {
        invalid_response(
            "Fireworks returned a response that was not valid UTF-8",
...
fn response_too_large() -> LlmError {
    invalid_response(
        format!("Fireworks response exceeded the {MAX_RESPONSE_BYTES}-byte safety limit"),
        serde_json::Value::Null,
    )
}
