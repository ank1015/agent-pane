...
fn catalog_is_current_and_a_strict_allowlist() {
    assert_eq!(FIREWORKS_MODELS.len(), 6);
    assert!(find_model("accounts/fireworks/models/glm-5p3-flash").is_some());
    assert!(find_model("accounts/fireworks/models/glm-5p3").is_some());
...
fn cost_helper_prices_uncached_cached_and_output_tokens() {
    ...
    assert!((cost.total - 0.887).abs() < f64::EPSILON);
}
