...
pub struct ChatGptConfig {
    ...
    pub(crate) account_id: Zeroizing<String>,
...
impl ChatGptConfig {
    ...
    pub fn new(
        ...
        let account_id = Zeroizing::new(account_id.into());
...
fn validate_base_url(base_url: &str) -> Result<(), LlmError> {
    ...
    {
        return Err(invalid_config(
            "ChatGPT base URL must use HTTPS (loopback HTTP is allowed for local testing) and must not contain credentials, a query, or a fragment.",
        ));
    }
    Ok(())
}
